...
#def diffNbcoupValides(jeu): # c'est pas vraiement un repere : avnt cette fn et apres le taux de reussite change jst un petit peu et c'est un changement negatif 
 #   copie=game.getCopieJeu(jeu)
  #  copie[1]=  copie[1]%2+1  
   # return len(game.getCoupsValides(jeu)) - len(game.getCoupsValides(jeu))
...
""" if (game.getJoueur(jeu)==1):
        ...
        ligneAdv=1
        for i in range(6):
            if (plateau[ligneJoueur][6-plateau[ligneAdv][i]-1]>2):
                nb= nb + 1
    else:
        ligneJoueur=1
        ligneAdv=0
        for i in range(6):
            for c in range(plateau[ligneAdv][i]):
                if (c < 6):
                    c= c+1
                else: 
                    c= c-1
            if (plateau[ligneJoueur][c]>2):
                nb= nb + 1
