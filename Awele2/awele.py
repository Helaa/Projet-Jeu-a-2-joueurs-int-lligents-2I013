...
def finJeu(jeu):
    ...
    diff=[item for item in plateau[not(joueur-1)] if (item not in Lnull) ] # liste des différences entre le camp du joueur advrese et la liste nulle 
    return (jeu[2]==[] and diff==[] )or (game.getCoupsValides(jeu) ==[])
