...
def decision(jeu, p=0,signe=1,alpha=-10000,beta=10000):
    ...
    for cp in coups:
        ...
        v=-estimation(copie,p+1,-signe,-beta,-alpha)
...
def estimation(jeu,p,signe,alpha,beta):
    if (game.finJeu(jeu)):
    ...
    for cp in coups:
        copie=game.getCopieJeu(jeu)
        game.joueCoup(copie,cp)
        v=-estimation(copie,p+1,-signe,-alpha-1,-alpha)
        if (alpha<v and v<beta):
            v=-estimation(copie,p+1,-signe,-beta,-v)
